...
public class AuthController : ControllerBase
{
    ...
    public AuthController(OpenIddictClientService service)
        => _service = service;


    [HttpGet("~/login")]
    public ActionResult LogIn(string returnUrl = "/sso", string provider = "keycloak")
    {
        Thread.Sleep(3000);

        var client = _service.GetClientRegistrationByIdAsync(provider).GetAwaiter().GetResult();
        ...
        else
        {
            ...
            })
            {
                ...
                RedirectUri = Url.IsLocalUrl(returnUrl) ? returnUrl : "/sso"
            };
    ...
    public async Task<ActionResult> LogInCallback()
    {
        ...
        var identity = new ClaimsIdentity(authenticationType: "ExternalLogin", nameType: "name", roleType: "");
